"""
PrimeHaul OS — Social Media Auto-Pilot (B2B).

Generates branded social media content targeting removal company owners via
OpenAI, creates images with Pillow, publishes to Facebook, Instagram, X, and
LinkedIn on a schedule, and tracks engagement.
"""
...
B2B_CONTENT_PILLARS = {
    "growth": {
        ...
        "prompts": [
            "a practical tip for removal companies to win more jobs",
            "how removal companies can grow revenue using technology",
            "a marketing tip for removal company owners to attract customers",
        ],
    },
    "efficiency": {
        "label": "Efficiency",
        "prompts": [
            "how AI-powered quoting saves removal companies hours per week",
            "a productivity tip for running a more efficient removals business",
            "how digital surveys replace in-home visits and save fuel costs",
        ],
    },
    "insight": {
        "label": "Industry Insight",
        "prompts": [
            "a UK removals industry trend or market insight for 2026",
            "how the removals industry is changing with technology",
            "a surprising statistic about the UK house moving market",
        ],
    },
    "feature": {
        "label": "Product Feature",
        "prompts": [
            "how PrimeHaul's AI photo scanning creates instant inventories",
            "the benefit of customers self-surveying via a branded link",
            "how PrimeHaul's boss dashboard gives removal companies full control",
        ],
    },
    "proof": {
        "label": "Social Proof",
        "prompts": [
            "a success story framing for how AI quotes help removal companies",
            "a results-oriented claim about faster quoting with PrimeHaul",
            "how removal companies save time with automated customer surveys",
        ],
    },
}

PLATFORM_SPECS = {
    "facebook": {"max_chars": 2000, "hashtag_count": 5, "tone": "authoritative yet approachable"},
    "instagram": {"max_chars": 2200, "hashtag_count": 15, "tone": "visual and results-driven with emojis"},
    "x": {"max_chars": 275, "hashtag_count": 3, "tone": "punchy and bold"},
    "linkedin": {"max_chars": 1300, "hashtag_count": 5, "tone": "professional thought-leadership"},
}
...
def _pick_content_type(config: SocialConfig) -> str:
    ...
    mix = config.content_mix or {"growth": 25, "efficiency": 25, "insight": 20, "feature": 20, "proof": 10}
    types = list(mix.keys())
...
def generate_post_content(
    ...
    """
    Use OpenAI to generate a B2B social media post.
    ...
    pillar = B2B_CONTENT_PILLARS.get(content_type, B2B_CONTENT_PILLARS["growth"])
    topic_prompt = random.choice(pillar["prompts"])
    spec = PLATFORM_SPECS.get(platform, PLATFORM_SPECS["facebook"])
    tone = config.tone if config and config.tone else "professional_authoritative"

    system_msg = (
        "You are a B2B social media copywriter for PrimeHaul, a UK-based SaaS platform "
        "that helps removal companies manage AI-powered customer surveys and quotes. "
        "Our audience is removal company owners and managers. "
        f"Brand voice: confident, data-driven, results-oriented. Tone: {tone.replace('_', ' ')}."
    )
    ...
    user_msg = (
        ...
        f"- Include {spec['hashtag_count']} relevant hashtags (include #removals #moving)\n"
        f"- Tone: {spec['tone']}\n"
        f"- Include a CTA about PrimeHaul helping removal companies with AI-powered quotes\n"
        f"- UK English spelling\n"
        f"- Target audience: removal company owners in the UK\n\n"
...
def generate_social_image(
    ...
    """
    Generate a 1080x1080 branded social media image (B2B style).
    ...
    try:
        ...
        # Sub-brand
        sub_brand_font = _get_font(18)
        draw.text((60, 72), "For Removal Companies", fill=BRAND_MUTED, font=sub_brand_font)

        # Content type pill
        pill_font = _get_font(20)
        pillar_label = B2B_CONTENT_PILLARS.get(content_type, {}).get("label", content_type.title())
        pill_w = draw.textlength(pillar_label, font=pill_font) + 32
        draw.rounded_rectangle(
            [(60, 105), (60 + pill_w, 135)],
            radius=15,
            fill=(*BRAND_ACCENT, 40),
            outline=BRAND_ACCENT,
        )
        draw.text((76, 108), pillar_label, fill=BRAND_ACCENT, font=pill_font)
        ...
        for i, line in enumerate(wrapped.split("\n")):
            draw.text((60, y_start + i * 66), line, fill=BRAND_WHITE, font=headline_font)

        # Sub text
        if sub_text:
            sub_font = _get_font(28)
            sub_wrapped = textwrap.fill(sub_text, width=42)
            sub_y = y_start + (len(wrapped.split("\n")) * 66) + 40
            for i, line in enumerate(sub_wrapped.split("\n")):
                draw.text((60, sub_y + i * 38), line, fill=BRAND_MUTED, font=sub_font)

        # Bottom CTA bar
        draw.rectangle([(0, 980), (1080, 1080)], fill=(20, 20, 22))
        cta_font = _get_bold_font(24)
        draw.text((60, 1010), "AI-powered quotes for your removal company", fill=BRAND_ACCENT, font=cta_font)
...
def generate_weekly_content():
    """Generate a week's worth of B2B social media posts (2/day x 7 days = 14 posts)."""
    logger.info("Starting weekly B2B content generation...")
    db = SessionLocal()
    try:
            ...
            for slot_idx in range(min(posts_per_day, len(posting_times))):
                ...
                for platform in active_platforms:
                    ...
                    post = SocialPost(
                        ...
                        content_pillar=B2B_CONTENT_PILLARS.get(content_type, {}).get("label", content_type),
        ...
        db.commit()
        logger.info(f"Weekly B2B content generation complete: {posts_created} posts created")
